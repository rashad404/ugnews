...
class NewsModel extends Model{
    ...
    public static function getList($limit=10){
        $array = self::$db->select("SELECT `id`,`time`,`title_".self::$def_language."`,`text_".self::$def_language."`,`thumb`,`image`,`price` FROM `".self::$tableName."` WHERE `status`=1 ORDER BY `id` DESC LIMIT $limit");
        return $array;
    }
