...
use Modules\admin\Models\old as TableModel;
use Models\LanguagesModel;
use Core\View;
use Helpers\Url;

class old extends MyController
{
    ...
    public static $imageFolder = 'seo_texts';
    ...
    public function search(){
        if($_POST){
            $word = $_POST['word'];
            $rows = $this->searchLikeFor($this->dataParams["cName"],['id','text'],$word);
        } else {
        ...
        if(isset($_POST["submit"]) && Csrf::isTokenValid() ){
            ...
            if($validator->isSuccess()){
                ...
                if($insert){
                    ...
                    return Url::redirect(MODULE_ADMIN."/seotexts");
                }else{
    ...
    protected function getPost($action = 'create')
    {
        ...
        $array["text"] = Security::safe($_POST['text']);
