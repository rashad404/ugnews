...
class CustomersModel extends Model{
    ...
    public static function getInputs(){
        ...
        $array[] = ['type'=>'text', 'name'=>self::$lng->get('Last Name'), 'index'=>true, 'key'=>'last_name', 'sql_type'=>'varchar(100)'];
        ...
        $array[] = ['type'=>'text', 'name'=>self::$lng->get('Budget'), 'index'=>true, 'key'=>'budget', 'sql_type'=>'decimal(10,2)'];
        $array[] = ['type'=>'date', 'name'=>self::$lng->get('Desired Move-in Date'), 'index'=>false, 'key'=>'desired_move_in', 'sql_type'=>'varchar(20)'];
        $array[] = ['type'=>'select2', 'name'=>self::$lng->get('Location'), 'index'=>true, 'key'=>'location', 'sql_type'=>'int(11)','data'=>self::getLocations()];
        $array[] = ['type'=>'select2', 'name'=>self::$lng->get('Apartment'), 'index'=>true, 'key'=>'apt_id', 'sql_type'=>'int(11)','data'=>self::getApartments()];
        $array[] = ['type'=>'select2', 'name'=>self::$lng->get('Apt model'), 'index'=>true, 'key'=>'apt_model', 'sql_type'=>'varchar(20)', 'data'=>self::getAptModels()];
        $array[] = ['type'=>'select2', 'name'=>self::$lng->get('Room type'), 'index'=>true, 'key'=>'room_type', 'sql_type'=>'varchar(20)','data'=>self::getAptRoomTypes()];
        $array[] = ['type'=>'select2', 'name'=>self::$lng->get('Source'), 'index'=>true, 'key'=>'source', 'sql_type'=>'varchar(50)','data'=>self::getSources()];
    ...
    public static function getName($id){
        $query = self::$db->selectOne("SELECT `first_name`,`last_name` FROM ".self::$tableName." WHERE `id`='".$id."'");
