<?php
namespace Modules\partner\Controllers;

use Core\Language;
use Helpers\Csrf;
use Helpers\Session;
use Models\LanguagesModel;
use Core\View;
use Helpers\Url;
use Modules\partner\Models\AptFutureModel;

class Aptfuture extends MyController
{

    public  $params = [
        'name' => 'aptfuture',
        'searchFields' => ['id','name'],
        'title' => 'Apt Future List',
        'position' => false,
        'status' => true,
        'actions' => true,
    ];

    public static $model;
    public static $lng;
    public static $def_language;
    public static $rules;
    public static $admin_role;

    public function __construct(){
        self::$def_language = LanguagesModel::getDefaultLanguage('partner');
        self::$admin_role = Session::get('user_session_role');
        self::$lng = new Language();
        self::$lng->load('partner');
        self::$rules = ['first_name' => ['required']];
        self::$model = new AptFutureModel($this->params);
        if(self::$admin_role!=1){
            $new_params = $this->params;
            $new_params['actions'] = false;
            $this->params = $new_params;
        }
        parent::__construct();
    }

    public function apt_com(){
        $model = self::$model;
        $model->parseApartmentsCom();
    }

    public function index(){
        $model = self::$model;
        if(isset($_POST['csrf_token']) && Csrf::isTokenValid()){
            $data['list'] = $model::search();
        }else{
            $data['list'] = $model::getList();
        }
        $data['params'] = $this->params;
        $data['inputs'] = $model::getInputs();
        $data['lng'] = self::$lng;
        View::renderPartner($this->params['name'].'/index',$data);
    }

    public function apartment($apt_id){
        $model = self::$model;
        if(isset($_POST['csrf_token']) && Csrf::isTokenValid()){
            $data['list'] = $model::search();
        }else{
            $data['list'] = $model::getListByApt($apt_id);
        }
        $data['params'] = $this->params;;
        $data['inputs'] = $model::getInputs();
        $data['lng'] = self::$lng;
        $data['apt_id'] = $apt_id;
        View::renderPartner($this->params['name'].'/index',$data);
    }

    public function add($apt_id){
        $model = self::$model;
        if(isset($_POST['csrf_token']) && Csrf::isTokenValid()){
            $modelArray = $model::add($apt_id);
            if(empty($modelArray['errors'])){
                Session::setFlash('success',self::$lng->get('Data has been saved successfully'));
                Url::redirect(MODULE_PARTNER."/".$this->params["name"].'/index/'.$apt_id);
            }else {
                Session::setFlash('error',$modelArray['errors']);
            }
        }
        $data['input_list'] = $model::getInputs($apt_id);
        $data['params'] = $this->params;;
        $data['item'] = '';
        $data['lng'] = self::$lng;
        $data['apt_id'] = $apt_id;
        View::renderPartner($this->params["name"].'/'.__FUNCTION__,$data);
    }

    public function update($bed_id){
        $model = self::$model;
        $data['params'] = $this->params;;
        $data['item'] = $model::getItem($bed_id);
        $data['input_list'] = $model::getInputs();
        $data['lng'] = self::$lng;

        if(isset($_POST['csrf_token']) && Csrf::isTokenValid()){
            $modelArray = $model::update($bed_id);
            if(empty($modelArray['errors'])){
                Session::setFlash('success',self::$lng->get('Data has been saved successfully'));
                Url::redirect(MODULE_PARTNER."/".$this->params["name"].'/index/'.$data['apt_id']);
            }else {
                Session::setFlash('error',$modelArray['errors']);
            }
        }
        View::renderPartner($this->params["name"].'/'.__FUNCTION__,$data);
    }

    public function up($id){
        $model = self::$model;
        $model::move($id,'up');
        Url::previous(MODULE_PARTNER."/".$this->params['name']);
    }
    public function down($id){
        $model = self::$model;
        $model::move($id,'down');
        Url::previous(MODULE_PARTNER."/".$this->params['name']);
    }
    public function status($id){
        $model = self::$model;
        $model::statusToggle($id);
        Url::previous(MODULE_PARTNER."/".$this->params['name']);
    }
    public function delete($id){
        $model = self::$model;
        $model::delete([$id]);
        Url::previous(MODULE_PARTNER."/".$this->params['name']);
    }
    public function operation(){
        $model = self::$model;
        if(isset($_POST["row_check"])){
            if(isset($_POST["delete"])){
                $model::delete();
            }elseif(isset($_POST["active"])){
                $model::status(1);
            }elseif(isset($_POST["deactive"])){
                $model::status(0);
            }
        }else{
            Session::setFlash('error','Please choose an action');
        }
        return Url::previous(MODULE_PARTNER."/".$this->dataParams["cName"].'/album');
    }


}