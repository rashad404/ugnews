...
class ApartmentsModel extends Model{
    ...
    public static function getList(){
        return self::$db->select("SELECT `id`,`title_".self::$def_language."`, `name`,`rent`,`address` FROM ".self::$tableName." WHERE `partner_id`='".self::$partner_id."'");
    }
    public static function getActiveList(){
        return self::$db->select("SELECT `id`,`title_".self::$def_language."`, `name`,`rent`,`address`,`category` FROM ".self::$tableName." WHERE `partner_id`='".self::$partner_id."' AND `status`=1");
    }
    ...
    public static function getAddress($id){
        $query = self::$db->selectOne("SELECT `full_address` FROM ".self::$tableName." WHERE `id`='".$id."'");
