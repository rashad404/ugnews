...
class BedsModel extends Model{
    ...
    public static function getTenants(){
        ...
        $room_list = $model->getList('LIMIT 0,1000000');
    ...
    public static function update($id){
        ...
        if ($validator->isSuccess()) {
            ...
            //Update tenant table;
                self::$db->update(self::$tableNameTenants,['apt_id'=>0,'room_id'=>0,'bed_id'=>0], ['bed_id'=>$id]);
                self::$db->update(self::$tableNameTenants,['apt_id'=>$old_data['apt_id'], 'room_id'=>$old_data['room_id'], 'bed_id'=>$old_data['id'], ], ['id'=>$post_data['tenant_id']]);
    ...
    public static function getEarnings($room_type){
        $array = self::$db->selectOne("SELECT SUM(`price`) as c FROM ".self::$tableName." WHERE `room_type`='".$room_type."' AND `status`=1 AND `tenant_id`>0 AND `partner_id`='".self::$partner_id."'");
        return $array['c'];
    }
    ...
    public static function getListNotice(){
        return self::$db->select("SELECT a.`id`,a.`room_id`,a.`apt_id`,a.`tenant_id`,a.`status`,a.`name_".self::$def_language."`,a.`price`, a.`available_date`, b.`name_".self::$def_language."` as `room_name` 
         FROM ".self::$tableName." as a INNER JOIN ".self::$tableNameRooms." as b ON a.`room_id`=b.`id` 
          WHERE  
           a.`partner_id`='".self::$partner_id."' AND a.`status`=1 AND a.`tenant_id`>0 AND a.`available_date`>'".date('Y-m-d')."'
    ...
    public static function getListVacant(){
        return self::$db->select("SELECT a.`id`,a.`room_id`,a.`apt_id`,a.`tenant_id`,a.`status`,a.`name_".self::$def_language."` as `name`,a.`price`, a.`available_date`, b.`name_".self::$def_language."` as `room_name` 
         FROM ".self::$tableName." as a INNER JOIN ".self::$tableNameRooms." as b ON a.`room_id`=b.`id` 
          WHERE  
           a.`partner_id`='".self::$partner_id."' AND a.`status`=1 AND a.`tenant_id`=0 
    ...
    public static function getListByRoomVacant($room_id, $tenant_id=0){
        return self::$db->select("SELECT `id`,`name_".self::$def_language."` as `name` 
         FROM ".self::$tableName." WHERE `room_id`='".$room_id."' AND (`tenant_id`=0 or `tenant_id`='".$tenant_id."') and `status`=1 ORDER BY `position` DESC, `id` ASC ");
    }
