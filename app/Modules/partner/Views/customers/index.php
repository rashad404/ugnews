                                ...
                                <tr>
                                    ...
                                    <?php foreach ($data['inputs'] as $input_item):?>
                                        <?php if($input_item['index']):?>
                                            ...
                                            if($input_item['key']=='first_name'){
                                                $tenant_info = CustomersModel::getItem($item['id']);
                                                $input_value = '<a href="view/'.$item['id'].'">'.$tenant_info['first_name'].' '.$tenant_info['last_name'].'</a><br/>';
                                                $input_value .= '<span style="color:#496086;cursor:pointer;" onclick="copyFunction()">' .Format::phoneNumber($tenant_info['phone']).'</span>';

                                            }elseif($input_item['key']=='apt_id'){
                                                $input_value = ApartmentsModel::getName($input_value);
                                            }elseif($input_item['key']=='location'){
                                                $input_value = ApartmentsModel::getLocationName($input_value);
                                            }elseif($input_item['key']=='apt_model'){
                                                $input_value = ApartmentsModel::getModelName($input_value);
                                            }elseif($input_item['key']=='room_type'){
                                                $input_value = ApartmentsModel::getRoomTypeName($input_value);
                                            }elseif($input_item['key']=='source'){
                                                $input_value = CustomersModel::getSourceName($input_value);
                                            }
                                            ?>
