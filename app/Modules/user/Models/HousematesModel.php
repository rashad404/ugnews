...
class HousematesModel extends Model
{
    ...
    public static function countList(){
        $count = self::$db->selectOne("SELECT count(`id`) as countList FROM ".self::$tableName." WHERE `apt_id`='" . self::$user_id . "' AND `partner_id`='" . self::$partner_id . "'");
    ...
    {
        $user_info = UserModel::getItem(self::$user_id);
        if($user_info['apt_id']>0) {
            return self::$db->select("SELECT * FROM " . self::$tableName . " WHERE `apt_id`='" . $user_info['apt_id'] . "' AND `partner_id`='" . self::$partner_id . "' ORDER BY `id` DESC");
        }else{
            return array();
        }
    }
