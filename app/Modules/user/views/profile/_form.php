            ...
            <div class="col-sm-4">
                <div class="form-group">
                    ...
                    <?php if($value['type']=='select_box'):?>
                        <select <?=(key_exists('readonly', $value))?'disabled="true"':''?> name="<?=$value['key']?>" class="form-control ">
                    ...
                    <?php elseif($value['type']=='select2'):?>
                        <select <?=(key_exists('readonly', $value))?'disabled="true"':''?> name="<?=$value['key']?>" class="select2 form-control">
                            <?php foreach($value['data'] as $data):?>
                                <option <?=$item&&$item[$value['key']]==$data['key']?'selected':''?> value="<?=$data['key']?>" <?=$data['disabled']?>><?=$data['name']?></option>
                            <?php endforeach;?>
                        </select>
                    <?php else: ?>
                        <input <?=(key_exists('readonly', $value))?'readonly':''?> class="form-control admininput" type="<?=$value['type']?>" placeholder="" name="<?=$value['key']?>" value="<?=$item?$item[$value['key']]:''?>">
                    <?php endif; ?>
